...
function analisisLexico(texto){
    ...
    while(indiceActual <= texto.length){      
        ...
        } else if(tokenSalto == true) {
            identifyLine(texto.charCodeAt(indiceActual));
        }
        else{
            lexemaActual = "";
...
function identifyLine(caracterAscii) {
    if(caracterAscii == 10) {
        line++;
        lines.push(line);
    }
}
 //determinacion de identificador
...
//determinacion de tipo de numero
...
/*
Metodo encargado de imprimir en la tabla el resumen del analisis lexico
*/
function writeOutput() {
    var salida = "";
    for(var i = 0; i < lexemas.length -1; i++){
        salida = "<tr>" + "<td>" + lexemas[i] + "</td>" + "<td>" + tokens[i] + "</td>" + "<td>" 
        + lines[i] + "</td>" + "</tr>";
    }   
    table.innerHTML = table.innerHTML + salida;
}
